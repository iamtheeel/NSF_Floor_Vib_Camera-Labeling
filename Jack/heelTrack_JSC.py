...
from distance_position import find_dist_from_y as dist
from distance_position import find_resolution_px_dist as res
...
for i in range(clipRunFrames): # Go through each frame this many times
    ...
    if len(pose_landmarker_result.pose_landmarks) > 0:
        ...
        left_heel_dist = dist(left_heel_y_px)
        left_heel_res = res(left_heel_y_px)

        # Right heel (landmarks[30])
        right_heel_y_px = int((landmarks[30].y * h))
        #right_heel_dist = 7927.4586 / (right_heel_y_px + 85.6982) + -1.0522
        right_heel_dist = dist(right_heel_y_px)
        right_heel_res = res(right_heel_y_px)

        print(f"Left heel: y={left_heel_y_px}, distance={left_heel_dist:.2f} meters, resolution= {left_heel_res}")
        print(f"Right heel: y={right_heel_y_px}, distance={right_heel_dist:.2f} meters, resolution= {right_heel_res}")
